...
output_key = "output/pii_content_job_metadata.json"
...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        ...
        job_name = f'pii_content_job_{now:%Y-%m-%d-%H-%M}'
        ...
        try:
            ...
            response = comprehend.start_pii_entities_detection_job(
                ...
                # CHANGE 2: Configure how you want the redaction to look
                RedactionConfig={
                    'PiiEntityTypes': ['ALL'], # Redact all types found
                    'MaskMode': 'REPLACE_WITH_PII_ENTITY_TYPE' # This swaps the text for the Label
                    #'MaskCharacter': '*',      # Replace characters with *
                    #'MaskMode': 'MASK_CHARACTER' # Options: 'MASK_CHARACTER' or 'REPLACE_WITH_PII_ENTITY_TYPE'
                },
                Mode='ONLY_REDACTION',  # Use 'ONLY_OFFSETS' to get the locations of PII, or 'ONLY_REDACTION' to mask it
                JobName=job_name,
                LanguageCode='en',
                DataAccessRoleArn=data_arn
            )
            
            logger.info(f"Started PII Content Job: {response['JobId']}")
